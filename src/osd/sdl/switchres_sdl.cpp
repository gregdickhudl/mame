...
bool switchres_resolution_change(sdl_window_info *window)
{
	...
	window->m_win_config.width = best_mode->type & MODE_ROTATED? best_mode->vactive : best_mode->hactive;
	window->m_win_config.height = best_mode->type & MODE_ROTATED? best_mode->hactive : best_mode->vactive;
	...
	if (memcmp(&previous_mode, best_mode, sizeof(modeline) - sizeof(mode_result)))
		window->window_changeres(machine);
