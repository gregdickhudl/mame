...
bool adl_display_mode_info_to_modeline(ADLDisplayModeInfo *dmi, modeline *m)
{
	...
	m->hsync     = (dt.sTimingFlags & ADL_DL_TIMINGFLAG_H_SYNC_POLARITY)? 1 : 0;
	m->vsync     = (dt.sTimingFlags & ADL_DL_TIMINGFLAG_V_SYNC_POLARITY)? 1 : 0;
...
bool adl_set_modeline(char *target_display, modeline *m, int update_mode)
{
	...
	dt->sTimingFlags     = (m->interlace? ADL_DL_TIMINGFLAG_INTERLACED : 0) | (m->hsync? ADL_DL_TIMINGFLAG_H_SYNC_POLARITY : 0) | (m->vsync? ADL_DL_TIMINGFLAG_V_SYNC_POLARITY : 0);
	dt->sHTotal          = m->htotal;
	...
	if (update_mode & MODELINE_UPDATE) adl_get_modeline(target_display, m);
