...
bool ati_get_modeline(modeline *mode)
{
	...
	DWORD length;
	bool found = false;
	int refresh_label = mode->refresh * win_interlace_factor(mode);
	int vfreq_incr = 0;
	...
	if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, m_device_key, 0, KEY_ALL_ACCESS, &hKey) == ERROR_SUCCESS)
	{
		...
		if (found)
		{
			...
			sprintf (lp_name, "DALR6 CRT%dx%dx0x%d", mode->width, mode->height, refresh_label + vfreq_incr);
...
/*
			...
			if (RegQueryValueExA(hKey, lp_name, NULL, NULL, (LPBYTE)lp_data2, &length) == ERROR_SUCCESS && length == sizeof(lp_data2))
			{
				...
				if (RegSetValueExA(hKey, lp_name, 0, REG_BINARY, (LPBYTE)lp_data2, 32) != ERROR_SUCCESS)
					osd_printf_verbose("Failed saving registry entry for polarity information\n");
			}
*/
		}
		else
...
static int win_interlace_factor(modeline *mode)
{
	if (win_version > 5 && mode->interlace && mode->refresh < 40)
		return 2;
