...
bool switchres_init_osd(running_machine &machine)
{
	...
	if (machine.options().modeline_generation())
		custom_video_init(m_device_name, m_device_id, &desktop_mode, user_mode,
...
bool switchres_modeline_setup(running_machine &machine)
{
	...
	{
		custom_video_update_timing(best_mode);
	...
	set_option_osd(machine, OPTION_SYNCREFRESH, !options.triple_buffer());
	...
	set_option_osd(machine, OSDOPTION_FILTER, ((best_mode->result.weight & R_RES_STRETCH || best_mode->interlace) && (!strcmp(options.video(), "auto") || !strcmp(options.video(), "d3d") || !strcmp(options.video(), "d3d9ex"))));
	...
	cs->cleanstretch = !options.cleanstretch()? !(best_mode->result.weight & R_RES_STRETCH) : options.cleanstretch();
...
bool switchres_resolution_change(win_window_info *window)
{
	...
	if (options.changeres())
	{
		...
		if (memcmp(&previous_mode, best_mode, sizeof(modeline) - sizeof(mode_result)))
			SendMessage(window->m_hwnd, WM_USER_CHANGERES, 0, 0);
	}
	else
	{
		window->m_win_config.width = game->width;
		window->m_win_config.height = game->height;
	}
...
//============================================================
// switchres_get_aspect
//============================================================

float switchres_get_aspect(modeline *mode, float aspect)
{
	float aspect_corrector = 1.0f;

	if (mode && mode->hactive)
	{
		aspect_corrector = ((float)mode->width / (float)mode->height) / ((float)mode->hactive / (float)mode->vactive);
		if (mode->type & MODE_ROTATED) aspect_corrector = 1.0 / aspect_corrector;
		aspect *= aspect_corrector;
	}
...
int display_restore_desktop_mode(int flags)
{
	if (ChangeDisplaySettingsExA(m_device_name, &desktop_devmode, NULL, CDS_TEST | CDS_UPDATEREGISTRY, 0) == DISP_CHANGE_SUCCESSFUL)
		return true;
